...
(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      factory((global.ChartBoxPlot = {}), global.Chart, global.Chart.helpers));
  ...
  function interpolateKdeCoords(from, to, factor) {
    if (Array.isArray(from) && Array.isArray(to)) {
      return to.map((t, i) => ({
        ...
        estimate: interpolators.number(
          from[i] ? from[i].estimate : null,
          t.estimate,
          factor
        ),
      }));
  ...
  class StatsBase$1 extends chart_js.BarController {
    ...
    _toStringStats(b) {
      const f = (v) =>
        v == null
          ? "NaN"
          : helpers.formatNumber(v, this.chart.options.locale, {});
      return `(min: ${f(
        b.min
      )}, 25% quantile: ${f(b.q1)}, median: ${f(b.median)}, mean: ${f(b.mean)}, 75% quantile: ${f(b.q3)}, max: ${f(b.max)})`;
    }
    updateElement(rectangle, index, properties, mode) {
      ...
      this._transformStats(properties, parsed, (v) =>
        reset ? base : scale.getPixelForValue(v, index)
      );
  ...
  const baseOptionKeys = (() =>
    Object.keys(baseDefaults).concat(Object.keys(baseRoutes)))();
  ...
  const boxOptionsKeys = baseOptionKeys.concat([
    "medianColor",
  ...
  class Violin extends StatsBase {
    ...
    _getBounds(useFinalPosition) {
      if (this.isVertical()) {
        const { x, width, min, max } = this.getProps(
          ["x", "width", "min", "max"],
          useFinalPosition
        );
        const x0 = x - width / 2;
      ...
      const { y, height, min, max } = this.getProps(
        ["y", "height", "min", "max"],
        useFinalPosition
      );
      const y0 = y - height / 2;
    ...
    baseDefaults$1(boxOptionsKeys),
    {
      animations: {
        numbers: {
          type: "number",
          properties:
            chart_js.BarController.defaults.animations.numbers.properties.concat(
              [
                "q1",
                "q3",
                "min",
  ...
  BoxPlotController.overrides = helpers.merge({}, [
  ...
  class BoxPlotChart extends chart_js.Chart {
    constructor(item, config) {
      super(
        item,
        patchController("boxplot", config, BoxPlotController, BoxAndWiskers, [
  ...
  ViolinController.overrides = helpers.merge({}, [
    chart_js.BarController.overrides,
    defaultOverrides(),
  ]);
