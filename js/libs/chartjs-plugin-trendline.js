    ...
    beforeDraw: function(chartInstance) {
        ...
        chartInstance.data.datasets.forEach(function(dataset, index) {
            ...
            if (!dataset._meta[metaIndex].hidden) {
                ...
                fitterMetaData = fitterMetaData.concat(chartInstance.getDatasetMeta(index).data);
            }
        });

        if (fitterData.length > 0 && fitterMetaData.length > 0) {
            addFitter(chartInstance.chart, fitterMetaData, fitterData, xScale, yScale);
        }
    }
};
...
function addFitter(chart, metaData, data, xScale, yScale) {
    ...
    // Define vars
    var minX = data.reduce(function(prev, curr) {
        return prev.x.valueOf() < curr.x.valueOf() ? prev : curr;
    }).x.valueOf();
    var maxX = data.reduce(function(prev, curr) {
